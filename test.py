...
# ------------------------------------------------------------------  
# Autonomous Catalogue Expansion — assisted / automatic + discovery  
# ------------------------------------------------------------------  
  
print("\n=== Testing Catalogue Expansion (assisted/automatic) ===")  
  
from core.config import Config as _Config  
from metadata.services.discovery_loop import DiscoveryLoop as _DiscoveryLoop  
from metadata.services.enrichment.archive_search_source import (  
    ArchiveSearchSource as _ArchiveSearchSource,  
)  
...
def _make_loop(mode, cands, url, cap=0):  
    ...
    loop = _DiscoveryLoop(config=cfg, source_search=_FakeSearch(url))  
...
_off = _DiscoveryLoop(config=_Config(), source_search=_FakeSearch("x"))  
...
assert _ArchiveSearchSource().find_source_url("") is None  
